...
import { subtractVectors, getLookAtLocation, swapSlots } from '../utils/helpers.js'
...
class FP {
	...
	*////////////////////////////
	...
	look(sender) {
		...
		const loc = subtractVectors(b.block.location, FP.test.worldLocation({ x: 0, y: 0, z: 0 }));
	...
	breakblock() {
		this.#getBlockFromViewDirection((b) => {
		//const loc = FP.test.relativeBlockLocation(b.location);
		const loc = subtractVectors(b.location, FP.test.worldLocation({ x: 0, y: 0, z: 0 }));
